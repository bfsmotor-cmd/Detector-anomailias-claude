...
with st.sidebar:
    ...
    st.divider()
    st.subheader("📘 Fit Meta Ads")
    meta_window_days = st.slider("Ventana de análisis (días)", 7, 90, 30, key="meta_window")
    meta_cpc_threshold = st.number_input(
        "CPC promedio mínimo (COP)",
        min_value=500, max_value=100_000, value=5_000, step=500,
        key="meta_cpc",
        help="Por encima de este CPC se considera un mercado caro en búsqueda (⭐⭐⭐).",
    )
    meta_clicks_threshold = st.slider(
        "Clics diarios máximos", 1, 30, 8, key="meta_clicks",
        help="Por debajo de este promedio diario de clics no hay volumen para optimizar (⭐⭐⭐⭐).",
    )
    meta_impr_threshold = st.slider(
        "Impresiones diarias máximas", 50, 2_000, 300, step=50, key="meta_impr",
        help="Por debajo de este promedio diario de impresiones la demanda de búsqueda es marginal (⭐⭐⭐).",
    )
    meta_min_days = st.slider(
        "Días mínimos con datos", 1, 14, 3, key="meta_min_days",
        help="Campañas más nuevas que esto se excluyen para evitar falsos positivos.",
    )
    meta_only_search = st.checkbox(
        "Solo campañas de Búsqueda",
        value=True,
        key="meta_only_search",
        help="Requiere la columna 'Tipo de campaña' en el export. Los criterios están pensados "
             "para Search; Display/PMax distorsionan el análisis.",
    )
...
# ── Tab 8: Fit Meta Ads ──────────────────────────────────────────────────────
with tab8:
    st.subheader(f"Fit Meta Ads — últimos {meta_window_days} días")
    st.caption(
        "Detecta cuentas cuyo mercado no encaja bien en Google Ads (búsqueda muy cara, sin "
        "volumen de clics o sin demanda de búsqueda) para proponer al cliente migrar el "
        "presupuesto a Meta Ads **antes** de que se moleste y se vaya. Objetivo: reducir churn."
    )

    cpc_thr_txt = f"{meta_cpc_threshold:,.0f}".replace(",", ".")
    with st.expander("¿Cómo se calcula el fit?", expanded=False):
        st.markdown(f"""
        Cada criterio suma estrellas cuando se cumple. El **Score** es el % de estrellas
        obtenidas sobre las disponibles.

        | Criterio | Umbral actual | Peso | Qué indica |
        |---|---|---|---|
        | CPC promedio alto | > \\${cpc_thr_txt} COP | ⭐⭐⭐ | Mercado caro: el cliente no puede competir en subasta |
        | Volumen de clics bajo | < {meta_clicks_threshold} clics/día | ⭐⭐⭐⭐ | Sin volumen no hay datos para optimizar ni para convertir |
        | Volumen de búsquedas bajo | < {meta_impr_threshold} impresiones/día | ⭐⭐⭐ | La demanda de búsqueda del producto es marginal |

        **Niveles:** 🔥 Alto = los 3 criterios · ⚠️ Medio ≥ 60% del score · 🟡 Bajo = una sola señal.

        Se analizan solo campañas activas con al menos **{meta_min_days} días** de datos,
        agregando por cuenta y por campaña. Los umbrales se ajustan en el sidebar.
        """)

    meta_thresholds = {
        "cpc_threshold": float(meta_cpc_threshold),
        "clicks_per_day_threshold": float(meta_clicks_threshold),
        "impressions_per_day_threshold": float(meta_impr_threshold),
    }

    fit_campaigns, fit_accounts, fit_notas = analyzer.compute_meta_ads_fit(
        df,
        window_days=meta_window_days,
        thresholds=meta_thresholds,
        min_days=int(meta_min_days),
        only_search=bool(meta_only_search),
    )

    for nota in fit_notas:
        st.info(f"ℹ️ {nota}")

    if fit_accounts.empty:
        st.warning("No hay datos suficientes para evaluar el fit con Meta Ads.")
    else:
        kpis_fit = analyzer.meta_fit_kpis(fit_accounts)

        k1, k2, k3, k4, k5 = st.columns(5)
        k1.metric("Cuentas evaluadas", kpis_fit["cuentas"])
        k2.metric("🔥 Fit alto", kpis_fit["alto"])
        k3.metric("⚠️ Fit medio", kpis_fit["medio"])
        k4.metric("🟡 Fit bajo", kpis_fit["bajo"])
        k5.metric(
            "Inversión en riesgo",
            f"${kpis_fit['inversion_en_riesgo']:,.0f}",
            help="Coste de las cuentas con fit alto o medio en la ventana analizada. "
                 "Es el presupuesto que conviene reconducir antes de perder al cliente.",
        )

        if kpis_fit["alto"] or kpis_fit["medio"]:
            st.warning(
                f"⚠️ {kpis_fit['alto'] + kpis_fit['medio']} cuenta(s) con señales de mala "
                f"adecuación a Google Ads. Prioriza la conversación con las de fit alto."
            )
        else:
            st.success("✅ Ninguna cuenta muestra señales claras para migrar a Meta Ads.")
        ...
        # Mapa de mercado: CPC vs volumen de clics, con los umbrales dibujados
        scatter_src = fit_accounts.copy()
        hover_cols = [c for c in ["Impr./día", "Coste", "Conversiones", "Campañas con señal"]
                      if c in scatter_src.columns]
        fig_fit = px.scatter(
            scatter_src,
            x="Clics/día", y="CPC promedio",
            color="Nivel fit", color_discrete_map=NIVEL_COLORS,
            size=scatter_src["Coste"].clip(lower=1),
            hover_name="Cuenta", hover_data=hover_cols,
            title="Mapa de mercado por cuenta — CPC vs. volumen de clics",
        )
        fig_fit.add_hline(
            y=meta_cpc_threshold, line_dash="dash", line_color="#ef4444",
            annotation_text=f"CPC ${cpc_thr_txt}",
        )
        fig_fit.add_vline(
            x=meta_clicks_threshold, line_dash="dash", line_color="#ef4444",
            annotation_text=f"{meta_clicks_threshold} clics/día",
        )
        fig_fit.update_layout(height=420, margin=dict(t=50, b=0))
        st.plotly_chart(fig_fit, use_container_width=True)
        st.caption(
            "🔎 El cuadrante **superior izquierdo** (CPC alto + pocos clics) es el candidato "
            "natural a migrar: caro y sin volumen."
        )

        # ── Tabla de cuentas con gestión comercial ──────────────────────────
        st.markdown("#### Cuentas candidatas")

        nf1, nf2 = st.columns([2, 1])
        with nf1:
            niveles_disp = [n for n in ["🔥 Alto", "⚠️ Medio", "🟡 Bajo", "— Sin fit"]
                            if n in set(fit_accounts["Nivel fit"])]
            default_niveles = [n for n in niveles_disp if n != "— Sin fit"]
            sel_niveles = st.multiselect(
                "Nivel de fit", niveles_disp,
                default=default_niveles or niveles_disp,
                key="meta_niveles",
            )
        with nf2:
            sel_cuenta_fit = st.selectbox(
                "Cuenta", ["Todas"] + sorted(fit_accounts["Cuenta"].unique().tolist()),
                key="meta_cuenta",
            )

        acc_view = fit_accounts.copy()
        if sel_niveles:
            acc_view = acc_view[acc_view["Nivel fit"].isin(sel_niveles)]
        if sel_cuenta_fit != "Todas":
            acc_view = acc_view[acc_view["Cuenta"] == sel_cuenta_fit]

        if acc_view.empty:
            st.info("Ninguna cuenta cumple los filtros seleccionados.")
        else:
            # Hidratar estado comercial guardado
            meta_store = comments_store.load_meta_fit_state()
            acc_view["Estado"] = acc_view["Cuenta"].map(
                lambda c: meta_store.get(c, {}).get("estado", "Pendiente")
            )
            acc_view["Asignado a"] = acc_view["Cuenta"].map(
                lambda c: meta_store.get(c, {}).get("asignado_a", "")
            )
            acc_view["Nota"] = acc_view["Cuenta"].map(
                lambda c: meta_store.get(c, {}).get("nota", "")
            )

            acc_cols = [
                "Score", "Nivel fit", "Cuenta", "Criterios cumplidos",
                "CPC promedio", "Clics/día", "Impr./día",
                "Coste", "Clics", "Conversiones",
                "Campañas con señal", "Campañas evaluadas",
                "Diagnóstico", "Acción sugerida",
                "Estado", "Asignado a", "Nota",
            ]
            acc_table = acc_view[[c for c in acc_cols if c in acc_view.columns]]

            edited_fit = st.data_editor(
                acc_table,
                column_config={
                    "Score": st.column_config.ProgressColumn(
                        "Score fit", min_value=0, max_value=100, format="%d",
                        width="small", pinned=True,
                    ),
                    "Nivel fit": st.column_config.TextColumn(disabled=True, width="small"),
                    "Cuenta": st.column_config.TextColumn(disabled=True, pinned=True),
                    "Criterios cumplidos": st.column_config.TextColumn(disabled=True, width="medium"),
                    "CPC promedio": st.column_config.NumberColumn(disabled=True, format="$%.0f"),
                    "Clics/día": st.column_config.NumberColumn(disabled=True, format="%.1f"),
                    "Impr./día": st.column_config.NumberColumn(disabled=True, format="%.0f"),
                    "Coste": st.column_config.NumberColumn(disabled=True, format="$%.0f"),
                    "Clics": st.column_config.NumberColumn(disabled=True, format="%.0f"),
                    "Conversiones": st.column_config.NumberColumn(disabled=True, format="%.1f"),
                    "Campañas con señal": st.column_config.NumberColumn(disabled=True, width="small"),
                    "Campañas evaluadas": st.column_config.NumberColumn(disabled=True, width="small"),
                    "Diagnóstico": st.column_config.TextColumn(disabled=True, width="large"),
                    "Acción sugerida": st.column_config.TextColumn(disabled=True, width="large"),
                    "Estado": st.column_config.SelectboxColumn(
                        "Estado", options=comments_store.META_FIT_ESTADOS,
                        default="Pendiente", required=True,
                    ),
                    "Asignado a": st.column_config.TextColumn("Asignado a", max_chars=80),
                    "Nota": st.column_config.TextColumn("Nota", max_chars=500, width="large"),
                ...
                hide_index=True,
                use_container_width=True,
                height=min(600, 60 + len(acc_table) * 38),
                key="meta_fit_editor",
            )

            # Persistir cambios de gestión comercial
            cambios_fit = []
            for _, row in edited_fit.iterrows():
                prev = meta_store.get(row["Cuenta"], {})
                new_est = row.get("Estado", "Pendiente") or "Pendiente"
                new_asig = (row.get("Asignado a") or "").strip()
                new_nota = (row.get("Nota") or "").strip()
                if (
                    new_est != prev.get("estado", "Pendiente")
                    or new_asig != prev.get("asignado_a", "")
                    or new_nota != prev.get("nota", "")
                ):
                    cambios_fit.append({
                        "cuenta": row["Cuenta"],
                        "estado": new_est,
            ...
            if cambios_fit:
                comments_store.sync_meta_fit_bulk(cambios_fit)
                st.toast(f"💾 {len(cambios_fit)} cuenta(s) actualizada(s)", icon="✅")

            st.download_button(
                "📥 Descargar cuentas candidatas (CSV)",
                edited_fit.to_csv(index=False).encode("utf-8"),
                f"fit_meta_ads_cuentas_{latest_date.strftime('%Y%m%d')}.csv",
                "text/csv",
            )

        # ── Detalle por campaña ─────────────────────────────────────────────
        st.markdown("#### Detalle por campaña")
        camp_view = fit_campaigns.copy()
        if sel_cuenta_fit != "Todas":
            camp_view = camp_view[camp_view["Cuenta"] == sel_cuenta_fit]
        if sel_niveles:
            camp_view = camp_view[camp_view["Nivel fit"].isin(sel_niveles)]

        if camp_view.empty:
            st.info("Ninguna campaña cumple los filtros seleccionados.")
        else:
            camp_cols = [
                "Score", "Nivel fit", "Cuenta", "Campaña", "Criterios cumplidos",
                "CPC promedio", "Clics/día", "Impr./día",
                "Días", "Clics", "Impresiones", "Coste", "Conversiones", "Diagnóstico",
            ]
            st.dataframe(
                camp_view[[c for c in camp_cols if c in camp_view.columns]],
                use_container_width=True,
                hide_index=True,
                height=min(500, 60 + len(camp_view) * 36),
            )
            st.download_button(
                "📥 Descargar detalle por campaña (CSV)",
                camp_view.drop(columns=[c for c in camp_view.columns if c.startswith("_")])
                .to_csv(index=False).encode("utf-8"),
                f"fit_meta_ads_campanas_{latest_date.strftime('%Y%m%d')}.csv",
                "text/csv",
                key="meta_dl_camp",
            )

        # ── Mensaje para hablar con el cliente ──────────────────────────────
        candidatas = fit_accounts[fit_accounts["Nivel fit"].isin(["🔥 Alto", "⚠️ Medio"])]
        if not candidatas.empty:
            st.markdown("#### Guion para la conversación con el cliente")
            st.caption(
                "Argumentario listo para copiar, con los datos de la cuenta. Ajusta el tono "
                "antes de enviarlo."
            )
            for _, acc in candidatas.iterrows():
                with st.expander(f"{acc['Nivel fit']} — {acc['Cuenta']} (score {acc['Score']})"):
                    def _co(val, prefix=""):
                        return prefix + f"{val:,.0f}".replace(",", ".")

                    impr_line = (
                        f"- Volumen de búsquedas: {_co(acc['Impr./día'])} impresiones al día\n"
                        if "Impr./día" in acc.index and pd.notna(acc["Impr./día"]) else ""
                    )
                    cpc_txt = (
                        _co(acc["CPC promedio"], "$") if pd.notna(acc["CPC promedio"]) else "n/d"
                    )
                    lectura = (
                        "en búsqueda estamos compitiendo en un mercado con un costo de entrada "
                        "alto para el presupuesto actual y con poca demanda"
                        if acc.get("_cpc_alto", False)
                        else "la demanda de búsqueda para este servicio es muy baja, así que por "
                             "más que optimicemos no hay volumen suficiente que capturar"
                    )
                    guion = (
                        f"Hola, quería compartirte una lectura del último mes de la cuenta "
                        f"{acc['Cuenta']} en Google Ads.\n\n"
                        f"Los datos de los últimos {meta_window_days} días muestran esto:\n"
                        f"- Costo por clic promedio: {cpc_txt}\n"
                        f"- Volumen de clics: {acc['Clics/día']:.1f} clics al día\n"
                        f"{impr_line}"
                        f"\n{acc['Diagnóstico']}\n\n"
                        f"En la práctica esto significa que {lectura}. Antes de seguir invirtiendo "
                        f"ahí, nuestra recomendación es evaluar mover parte del presupuesto a Meta "
                        f"Ads, donde en vez de esperar a que alguien busque el producto podemos "
                        f"generar la demanda y llegar al público objetivo a un costo por resultado "
                        f"más bajo.\n\n"
                        f"¿Te parece si agendamos 20 minutos para revisarlo juntos y definir un "
                        f"piloto?"
                    )
                    st.text_area(
                        "Mensaje", guion, height=300, key=f"meta_msg_{acc['Cuenta']}",
                        help="Selecciona el texto y cópialo.",
                    )
