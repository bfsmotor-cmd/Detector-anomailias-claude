...
# ─── Fit Meta Ads: cuentas candidatas a migrar de Google Ads a Meta Ads ──────
#
# Hipótesis de negocio: cuando un mercado es demasiado caro en búsqueda, el
# cliente no tiene presupuesto para competir en él, o simplemente no existe
# volumen de búsqueda para lo que quiere pautar, seguir insistiendo en Google
# Ads produce resultados pobres y termina en churn. Detectarlo a tiempo permite
# proponer una migración a Meta Ads (demanda generada, no capturada) antes de
# que el cliente se moleste.
#
# Cada criterio pesa en "estrellas" según su poder de señal:
#   ⭐⭐⭐   CPC promedio alto            → mercado caro
#   ⭐⭐⭐⭐ Pocos clics diarios          → sin volumen para optimizar
#   ⭐⭐⭐   Pocas impresiones diarias    → poca demanda de búsqueda

META_FIT_RULES = {
    "cpc_alto": {"stars": 3, "label": "CPC alto"},
    "pocos_clics": {"stars": 4, "label": "Pocos clics/día"},
    "poco_volumen": {"stars": 3, "label": "Bajo volumen de búsquedas"},
}

META_FIT_DEFAULTS = {
    "cpc_threshold": 5000.0,          # COP
    "clicks_per_day_threshold": 8.0,  # rango de referencia 5–8 clics/día
    "impressions_per_day_threshold": 300.0,
}

META_FIT_MAX_STARS = sum(r["stars"] for r in META_FIT_RULES.values())

SEARCH_CAMPAIGN_HINTS = ("búsqueda", "busqueda", "search")


def _meta_fit_level(score_pct: float) -> str:
    """Nivel de fit a partir del % de estrellas obtenidas sobre las disponibles."""
    if score_pct >= 90:
        return "🔥 Alto"
    if score_pct >= 60:
        return "⚠️ Medio"
...
def _meta_fit_texts(row, thresholds: dict, has_impressions: bool):
    """Devuelve (criterios_cumplidos, diagnóstico) legibles para el equipo."""
    criterios, diag = [], []

    if row["_cpc_alto"]:
        stars = "⭐" * META_FIT_RULES["cpc_alto"]["stars"]
        criterios.append(f"{stars} CPC alto")
        diag.append(
            f"CPC promedio de {_fmt_money(row['CPC promedio'])} supera el umbral de "
            f"{_fmt_money(thresholds['cpc_threshold'])}: mercado caro en búsqueda."
        )
    if row["_pocos_clics"]:
        stars = "⭐" * META_FIT_RULES["pocos_clics"]["stars"]
        criterios.append(f"{stars} Pocos clics/día")
        diag.append(
            f"Solo {row['Clics/día']:.1f} clics/día (umbral {thresholds['clicks_per_day_threshold']:.0f}): "
            f"volumen insuficiente para que el algoritmo optimice."
        )
    if has_impressions and row["_poco_volumen"]:
        stars = "⭐" * META_FIT_RULES["poco_volumen"]["stars"]
        criterios.append(f"{stars} Bajo volumen de búsquedas")
        diag.append(
            f"{row['Impr./día']:,.0f} impresiones/día (umbral {thresholds['impressions_per_day_threshold']:.0f}): "
            f"la demanda de búsqueda del producto es marginal.".replace(",", ".")
        )

    if not criterios:
        return "—", "La cuenta/campaña opera con costes y volumen razonables para Google Ads."
    return "  ·  ".join(criterios), " ".join(diag)


META_FIT_ACTIONS = {
    "🔥 Alto": (
        "Agendar conversación con el cliente: proponer migrar el presupuesto a Meta Ads. "
        "El mercado es caro y sin volumen; sostenerlo en búsqueda es un riesgo de churn."
    ),
    "⚠️ Medio": (
        "Revisar con el equipo de cuentas: hay señales claras de mala adecuación a búsqueda. "
        "Evaluar un piloto en Meta Ads con parte del presupuesto antes de migrar del todo."
    ),
    "🟡 Bajo": (
        "Monitorear. Solo una señal activa; primero agotar optimizaciones dentro de Google Ads."
    ),
    "— Sin fit": "Mantener en Google Ads.",
}


def _apply_meta_fit_rules(agg: pd.DataFrame, thresholds: dict, has_impressions: bool) -> pd.DataFrame:
    """Calcula métricas derivadas, estrellas, score y textos sobre un agregado."""
    dias = agg["Días"].replace(0, np.nan)

    agg["CPC promedio"] = (agg["Coste"] / agg["Clics"].replace(0, np.nan)).round(2)
    agg["Clics/día"] = (agg["Clics"] / dias).round(2)
    if has_impressions:
        agg["Impr./día"] = (agg["Impresiones"] / dias).round(0)

    agg["_cpc_alto"] = agg["CPC promedio"].fillna(0) > thresholds["cpc_threshold"]
    agg["_pocos_clics"] = agg["Clics/día"].fillna(0) < thresholds["clicks_per_day_threshold"]
    if has_impressions:
        agg["_poco_volumen"] = agg["Impr./día"].fillna(0) < thresholds["impressions_per_day_threshold"]
    else:
        agg["_poco_volumen"] = False

    max_stars = META_FIT_RULES["cpc_alto"]["stars"] + META_FIT_RULES["pocos_clics"]["stars"]
    if has_impressions:
        max_stars += META_FIT_RULES["poco_volumen"]["stars"]

    agg["Estrellas"] = (
        agg["_cpc_alto"].astype(int) * META_FIT_RULES["cpc_alto"]["stars"]
        + agg["_pocos_clics"].astype(int) * META_FIT_RULES["pocos_clics"]["stars"]
        + agg["_poco_volumen"].astype(int) * META_FIT_RULES["poco_volumen"]["stars"]
    )
    agg["Score"] = (agg["Estrellas"] / max_stars * 100).round(0).astype(int)
    agg["Nivel fit"] = agg["Score"].apply(_meta_fit_level)

    textos = agg.apply(lambda r: _meta_fit_texts(r, thresholds, has_impressions), axis=1)
    agg["Criterios cumplidos"] = [t[0] for t in textos]
    agg["Diagnóstico"] = [t[1] for t in textos]
    agg["Acción sugerida"] = agg["Nivel fit"].map(META_FIT_ACTIONS)

    return agg


def compute_meta_ads_fit(
    df: pd.DataFrame,
    window_days: int = 30,
    thresholds: dict | None = None,
    min_days: int = 3,
    ...
    """
    Evalúa qué cuentas/campañas son buen candidato para migrar a Meta Ads.

    Devuelve (campañas_df, cuentas_df, notas) donde `notas` lista las reglas
    desactivadas por columnas faltantes en el export.
    """
    thr = dict(META_FIT_DEFAULTS)
    if thresholds:
    ...
    recent_dates = _latest_n_dates(df, window_days)
    if not recent_dates:
        return empty, empty, notas

    recent = df[df["Día"].isin(recent_dates)].copy()

    # Solo campañas activas: la conversación con el cliente es sobre lo que corre hoy.
    if recent["_activa"].any():
        recent = recent[recent["_activa"]].copy()
    else:
        notas.append(
            "No se encontró columna de estado válida: se evalúan todas las campañas del archivo."
        )

    if "Clics" not in recent.columns or "Coste" not in recent.columns:
        notas.append("Faltan las columnas Clics y/o Coste: no se puede evaluar el fit.")
        return empty, empty, notas

    has_impressions = "_impresiones" in recent.columns
    if not has_impressions:
        notas.append(
            "Sin columna de Impresiones: la regla de volumen de búsquedas (⭐⭐⭐) "
            "queda desactivada y el score se calcula sobre los otros dos criterios."
        )

    if only_search and "_tipo_campana" in recent.columns:
        tipo = recent["_tipo_campana"].astype(str).str.lower()
        mask_search = tipo.str.contains("|".join(SEARCH_CAMPAIGN_HINTS), na=False)
        if mask_search.any():
            recent = recent[mask_search].copy()
        else:
            notas.append(
                "No se reconocieron campañas de tipo Búsqueda en la columna 'Tipo de campaña': "
                "se evalúan todos los tipos."
            )
    elif only_search:
        notas.append(
            "Sin columna 'Tipo de campaña': no se pudo restringir el análisis a campañas de Búsqueda."
        )

    if recent.empty:
        return empty, empty, notas

    def _aggregate(keys: list[str]) -> pd.DataFrame:
        spec = {
            "Días": ("Día", "nunique"),
            "Clics": ("Clics", "sum"),
            "Coste": ("Coste", "sum"),
        ...
        if has_impressions:
            spec["Impresiones"] = ("_impresiones", "sum")
        if "Conversiones" in recent.columns:
            spec["Conversiones"] = ("Conversiones", "sum")
        out = recent.groupby(keys).agg(**spec).reset_index()
        # Descartar filas sin actividad alguna: ya las cubren las pestañas de "sin movimiento".
        actividad = out["Clics"].fillna(0) + out["Coste"].fillna(0)
        if has_impressions:
            actividad = actividad + out["Impresiones"].fillna(0)
        return out[actividad > 0].copy()

    campaigns = _aggregate(["Cuenta", "Campaña"])
    accounts = _aggregate(["Cuenta"])

    if campaigns.empty:
        return empty, empty, notas

    descartadas = int((campaigns["Días"] < min_days).sum())
    if descartadas:
        notas.append(
            f"{descartadas} campaña(s) con menos de {min_days} días de datos quedaron fuera "
            f"del análisis para evitar falsos positivos."
        )
    campaigns = campaigns[campaigns["Días"] >= min_days].copy()
    accounts = accounts[accounts["Días"] >= min_days].copy()

    if campaigns.empty:
        return empty, empty, notas

    campaigns = _apply_meta_fit_rules(campaigns, thr, has_impressions)
    accounts = _apply_meta_fit_rules(accounts, thr, has_impressions)

    # Contexto de cuenta: cuántas de sus campañas disparan al menos un criterio.
    con_fit = (
        campaigns[campaigns["Estrellas"] > 0]
        .groupby("Cuenta")["Campaña"].nunique()
        .rename("Campañas con señal")
    )
    total_camp = campaigns.groupby("Cuenta")["Campaña"].nunique().rename("Campañas evaluadas")
    accounts = accounts.merge(total_camp, on="Cuenta", how="left").merge(
        con_fit, on="Cuenta", how="left"
    )
    accounts["Campañas con señal"] = accounts["Campañas con señal"].fillna(0).astype(int)
    accounts["Campañas evaluadas"] = accounts["Campañas evaluadas"].fillna(0).astype(int)

    campaigns = campaigns.sort_values(
        ["Score", "Coste"], ascending=[False, False]
    ).reset_index(drop=True)
    accounts = accounts.sort_values(
        ["Score", "Coste"], ascending=[False, False]
    ).reset_index(drop=True)

    return campaigns, accounts, notas


def meta_fit_kpis(accounts_df: pd.DataFrame) -> dict:
    """KPIs gerenciales de la pestaña Fit Meta Ads (a nivel cuenta)."""
    if accounts_df.empty:
        return {
            "cuentas": 0, "alto": 0, "medio": 0, "bajo": 0,
            "sin_fit": 0, "inversion_en_riesgo": 0.0,
        }
    nivel = accounts_df["Nivel fit"]
    en_riesgo = accounts_df[nivel.isin(["🔥 Alto", "⚠️ Medio"])]
    return {
        "cuentas": len(accounts_df),
        "alto": int((nivel == "🔥 Alto").sum()),
        "medio": int((nivel == "⚠️ Medio").sum()),
        "bajo": int((nivel == "🟡 Bajo").sum()),
        "sin_fit": int((nivel == "— Sin fit").sum()),
        "inversion_en_riesgo": float(en_riesgo["Coste"].sum()),
    }
